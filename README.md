...
一个轻盈、安静的 Windows LLM 翻译与查词工具。苹果式的留白与细腻控件，结合 Anthropic 风格的暖纸白、陶土色和无衬线排版。
...
- `设计/v0.1-CozyTranslator`：需求总结、界面预览与状态参考。

验证范围和实测性能见 `VALIDATION.md`。本轮未调用真实账户；协议夹具通过不等于各家账户已联调。

发布操作见 [发布步骤](docs/PUBLISHING.md)。
...
视觉参考：[Apple Human Interface Guidelines](https://developer.apple.com/design/human-interface-guidelines/layout)、[Anthropic](https://www.anthropic.com/)。本项目使用独立名称、图标与界面设计。
